...
def get_player_by_id(player_id: str) -> Optional[Dict]:
    ...
    return _get_collection().find_one({"player_id": player_id}, {"_id": 0})
...
def insert_players(players: List[Dict]) -> List[str]:
    """Insert a list of players with computed scouting and auction data."""
...
def recalculate_all_scores() -> int:
    """Recalculate scouting and auction data for every player."""
    from config import players_collection
