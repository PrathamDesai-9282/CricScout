"""Auction service layer — business logic for auction estimation."""

from typing import Dict, List, Optional
from config import players_collection
from models.auction_model import calculate_auction_points


def get_player_auction_estimate(player_id: str) -> Optional[Dict]:
    """
    Get auction point estimate for a single player.

    Args:
        player_id: The player's unique player_id (e.g. "P001").

    Returns:
        Dict with player info and auction breakdown, or None if not found.
    """
    player = players_collection.find_one({"player_id": player_id}, {"_id": 0})
    if not player:
        return None

    result = calculate_auction_points(player)
    return {
        "player": player["name"],
        "role": player.get("role"),
        "score": player.get("scouting_score"),
        "auction": result,
    }


def get_all_auction_estimates() -> List[Dict]:
    """
    Get auction estimates for all players, sorted by points descending.

    Returns:
        List of player auction summaries, sorted highest points first.
    """
    players = list(players_collection.find({}, {"_id": 0}))

    results = []
    for player in players:
        result = calculate_auction_points(player)
        results.append({
            "player": player["name"],
            "role": player.get("role"),
            "scouting_score": player.get("scouting_score"),
            "auction_points": result["auction_points"],
            "breakdown": result["breakdown"],
        })

    results.sort(key=lambda x: x["auction_points"], reverse=True)
    return results